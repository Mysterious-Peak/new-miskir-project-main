  ...
  <div class="p-2">
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" border stripe fit :data="productionBatchList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="Actions" align="center" fixed="right" class-name="action-column" :width="mainActionColumnWidth">
          <template #default="scope">
...
import { delQualityInspectionProcess } from '@/api/demo/qualityInspectionProcess';
...
const mainActionColumnWidth = computed(() => {
  const hasTwoButtons = productionBatchList.value.some((row: any) => !isViewFlag(row));
...
const handleAdd = async () => {
  ...
  proxy.$router.push({
    path: `qualityControlSubmit`,
    query: {
      mode: 'new' // 明确传递 new 模式
    }
  });
};
...
const handleUpdate = async (row?: ProductionBatchVO) => {
  const targetRow = row || productionBatchList.value.find((item) => item.productionBatchId === ids.value[0]);
  // 根据 isViewFlag 判断是 new 还是 edit 模式
  const mode = targetRow && isViewFlag(targetRow) ? 'edit' : 'new';

  proxy.$router.push({
    path: `qualityControlSubmit`,
    query: {
      productionBatchId: targetRow?.productionBatchId || ids.value[0],
      inspectionId: (targetRow as any)?.inspectionId,
      mode: mode
    }
  });
};
...
/** 删除按钮操作（按 Inspection ID 删除 Quality Inspection Process 记录） */
const handleDelete = async (row?: ProductionBatchVO) => {
  // 当前表格 Actions 中的 Delete 按钮会传入整行数据，这里直接取 inspectionId
  const _inspectionId = (row as any)?.inspectionId;

  if (!_inspectionId) {
    proxy?.$modal.msgError('Inspection ID is missing, cannot delete');
    return;
  }

  await proxy?.$modal.confirm('Are you sure delete this data?').finally(() => (loading.value = false));
  await delQualityInspectionProcess(_inspectionId);
  proxy?.$modal.msgSuccess('success');
  await getList();
  await getAllList();
};
