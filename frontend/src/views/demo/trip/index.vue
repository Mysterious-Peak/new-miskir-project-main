...
import { ShipmentOrderForm, ShipmentOrderQuery, ShipmentOrderVO } from '@/api/demo/shipmentOrder/types';
...
const handleBatchLinkSave = () => {
  batchLinkFormRef.value?.validate((valid: boolean) => {
    if (!valid) return;
    shipmentBatchLinks.value.push({
      shipmentBatchLinkId: `SBL-${String(shipmentBatchLinks.value.length + 1).padStart(3, '0')}`,
...
const handleUpdate = async (row?: ShipmentOrderVO) => {
  ...
  proxy.$router.push({
    path: '/logistics_delivery/tripAssignment',
    query: {
      shipmentId: row?.shipmentId || ids.value[0]
    }
  });
...
const isViewFlag = (data: { status?: string | number }) => {
  ...
  return status === ShipmentOrderStatus.Planned.toString();
};
</script>
