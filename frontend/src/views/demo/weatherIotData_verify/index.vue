    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          ...
          <!-- <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['demo:weatherIotData:edit']"
              >Edit</el-button
            >
          </el-col>
          <el-col :span="1.5">
          ...
          <!-- <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['demo:weatherIotData:export']">Export</el-button>
          </el-col> -->
        </el-row>
      </template>
...
import {
  ...
  delWeatherIotData,
  addWeatherIotData,
  updateWeatherIotData,
  listWeatherIotDataApprove
} from '@/api/demo/weatherIotData';
...
const getAllList = async () => {
  const res = await listWeatherIotDataApprove();
...
const getList = async () => {
  loading.value = true;
  const res = await listWeatherIotDataApprove(queryParams.value);
...
const handleAdd = () => {
  reset();
  dialog.visible = true;
  dialog.title = 'Add Weather & IoT Data';
};

/** Edit Button Action */
const handleUpdate = async (row?: WeatherIotDataVO) => {
  const _weatherId = row?.weatherId || ids.value[0];
  router.push({ name: 'WeatherIotDataApprove', params: { id: _weatherId } });
};
