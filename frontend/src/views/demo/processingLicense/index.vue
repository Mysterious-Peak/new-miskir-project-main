    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5" style="margin-left: auto; margin-right: 5px">
            <el-button type="primary" plain icon="Plus" @click="router.push({ name: 'ProcessingLicenseAdd' })" v-hasPermi="['demo:processingLicense:add']">Add</el-button>
          </el-col>
          <!-- <el-col :span="1.5">
      ...
      <el-table v-loading="loading" border :data="processingLicenseList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="Actions" align="center" fixed="right" :width="actionColumnWidth" class-name="action-column">
          <template #default="scope">
            <template
              v-if="scope.row.status === String(processingLicenseStatus.Draft) || scope.row.status === String(processingLicenseStatus.NeedRevision)"
            >
              <el-button
                class="table-action-btn table-edit-btn"
                icon="EditPen"
                @click="router.push({ name: 'ProcessingLicenseEdit', params: { id: scope.row.licenseId } })"
                v-hasPermi="['demo:processingLicense:edit']"
                >Edit</el-button
              >
              <el-button
                v-if="scope.row.status === String(processingLicenseStatus.Draft)"
                class="table-action-btn table-delete-btn"
                icon="Delete"
                @click="handleDelete(scope.row)"
                v-hasPermi="['demo:processingLicense:remove']"
                >Delete</el-button
              >
            </template>
            <el-button
              ...
              icon="View"
              @click="router.push({ name: 'ProcessingLicenseDetail', params: { id: scope.row.licenseId } })"
              >View</el-button
            >
          </template>
        </el-table-column>
...
const actionColumnWidth = computed(() => {
  const hasTwoButtons = processingLicenseList.value.some((row) => row.status === String(processingLicenseStatus.Draft));
  return hasTwoButtons ? 210 : 110;
});

// 查询区 License ID（已有记录 autoComplete）
...
const handleUpdate = async (row?: ProcessingLicenseVO) => {
  ...
  Object.assign(form.value, res.data);
  dialog.visible = true;
  dialog.title = 'Edit Processing License';
};
  ...
  processingLicenseFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.licenseId) {
        ...
        await addProcessingLicense(form.value).finally(() => (buttonLoading.value = false));
      }
