...
import { listCropMonitoring, getCropMonitoring, delCropMonitoring, addCropMonitoring, updateCropMonitoring } from '@/api/demo/cropMonitoring';
...
const getAllList = async () => {
  const res = await listCropMonitoring();
...
const getList = async () => {
  loading.value = true;
  const res = await listCropMonitoring(queryParams.value);
...
/** Add Button Action */
const handleAdd = () => {
  router.push({ name: 'CropMonitoringAdd' });
};

/** Edit Button Action */
const handleUpdate = (row?: CropMonitoringVO) => {
  const _monitorId = row?.monitorId || ids.value[0];
  router.push({ name: 'CropMonitoringEdit', params: { id: _monitorId } });
};
...
const handleView = (row?: CropMonitoringVO) => {
  const _monitorId = row?.monitorId || ids.value[0];
  router.push({ name: 'CropMonitoringDetail', params: { id: _monitorId } });
};
</script>
