...
const dialogMode = computed(() => {
  ...
  // Draft 状态或 undefined/null 可以编辑
  if (status === undefined || status === null || String(status) === String(AgroprocessorStatus.Draft)) {
    return 'all-edit';
  }
  return 'readonly';
});
...
const handleAdd = () => {
  reset();
  dialog.visible = true;
  dialog.title = "Review " + agroProcessorConfig.title;
}

/** 复核/查看按钮操作 */
const handleUpdate = (row?: AgroprocessorVO) => {
  const _processorId = row?.processorId || ids.value[0]
  // 根据状态跳转：Draft 可编辑显示 Review，其他状态只读显示 View
  if (row && canEdit(row)) {
    router.push({ name: 'AgroprocessorReview', params: { id: _processorId } });
  } else {
...
const showApprovedComment = (row: AgroprocessorForm): boolean => {
  ...
  // console.log(row.status != String(AgroprocessorStatus.Draft));
  // console.log(row.status != undefined && row.status != String(AgroprocessorStatus.Draft));
  // return row.status != undefined && row.status != String(AgroprocessorStatus.Draft);
...
const showApprovedCommentInput = (row: AgroprocessorForm): boolean => {
  ...
  // return row.status === String(AgroprocessorStatus.Draft) && (row.approvedComment != undefined && row.approvedComment != '');
  return false;
}
