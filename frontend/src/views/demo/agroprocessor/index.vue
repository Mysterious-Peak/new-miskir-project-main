      ...
      <template #header>
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5" style="margin-left: auto; margin-right: 5px">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['demo:agroprocessor:add']">Add</el-button>
          </el-col>
        </el-row>
      </template>
...
/** 新增按钮操作 */
const handleAdd = () => {
  router.push({ name: 'AgroprocessorAdd' });
}
...
const handleUpdate = (row?: AgroprocessorVO) => {
  ...
  if (row && canEdit(row)) {
    router.push({ name: 'AgroprocessorEdit', params: { id: _processorId } });
  } else {
...
const handleSubmitWithStatus = (status: AgroprocessorStatus) => {
  if (status === AgroprocessorStatus.NeedRevision) {
  }
  form.value.status = String(AgroprocessorStatus.Draft);
  submitForm();
};
...
const showApprovedComment = (row: AgroprocessorForm): boolean => {
  ...
  // 例如：只有已审批或已拒绝状态才展示
  return row.status === String(AgroprocessorStatus.Approved) || 
         row.status === String(AgroprocessorStatus.Rejected);
