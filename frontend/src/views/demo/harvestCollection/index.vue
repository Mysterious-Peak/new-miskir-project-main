  ...
  <div class="p-2">
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" stripe fit border :data="harvestCollectionList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="Standard Moisture (%)" align="center" prop="standardMoisturePct" :min-width="getColumnWidth('Standard Moisture (%)')" />
        <el-table-column label="Moisture Review" align="center" prop="standardMoistureNeedsReview" :min-width="getColumnWidth('Moisture Review')">
          <template #default="scope">
            <el-tag :type="scope.row.standardMoistureNeedsReview ? 'warning' : 'success'">
              {{ scope.row.standardMoistureNeedsReview ? 'Need review' : 'OK' }}
            </el-tag>
          </template>
        </el-table-column>
...
/** Add Button Action */
const handleAdd = () => {
  router.push({ name: 'HarvestCollectionAdd' });
};
...
const handleUpdate = (row?: HarvestCollectionVO) => {
  ...
  // {
  //   cropCodeLoading.value = true;
  //   cropCodeOptions.value = cropDataList.value.map((item) => ({
  //     value: item.cropCode,
  //     label: item.cropName
  //   }));
  //   cropCodeLoading.value = false;
  ...
  router.push({ name: 'HarvestCollectionEdit', params: { id: _id } });
};
...
const handleView = (row?: HarvestCollectionVO) => {
  const _id = row?.id || ids.value[0];
  router.push({ name: 'HarvestCollectionDetail', params: { id: _id } });
};
