...
// dialog for adding a batch link
...
const handleAdd = async () => {
  ...
  dialog.visible = true;
  dialog.title = 'Add Dispatch & Loading';
};
...
const handleUpdate = (row?: ShipmentOrderVO) => {
  ...
  if (isViewFlag(row)) {
    router.push({ name: 'DispatchLoadingDetail', params: { id: _shipmentId } });
  } else {
    router.push({ name: 'DispatchLoadingEdit', params: { id: _shipmentId } });
  }
};
...
const submitForm = () => {
  shipmentOrderFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.shipmentId) {
      ...
      } else if (shipmentBatchLinks.value.length > 0) {
        // 新建并且带有批次分配时，调用 createBatch 接口
        ...
        // 新建但没有批次分配时，仍然走原来的简单创建逻辑
        form.value.status = ShipmentOrderStatus.InTransit.toString();
        await addShipmentOrder(form.value).finally(() => (buttonLoading.value = false));
      }
      proxy?.$modal.msgSuccess('Operation successful');
      dialog.visible = false;
      await getList();
...
const handleDelete = async (row?: ShipmentOrderVO) => {
  ...
  await proxy?.$modal
    .confirm('Are you sure you want to delete the Dispatch & Loading with ID "' + _shipmentIds + '"?')
...
<style>
/* Popup — Shipment Information: two columns, narrower label column so controls are wider */
