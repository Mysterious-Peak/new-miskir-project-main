...
import { QualityInspectionVO, QualityInspectionQuery, QualityInspectionForm } from '@/api/demo/qualityInspection/types';
...
/** 新增按钮操作 */
const handleAdd = () => {
  router.push({ name: 'QualityInspectionAdd' });
};
...
const handleUpdate = (row?: QualityInspectionVO) => {
  ...
  router.push({ name: 'QualityInspectionEdit', params: { id: _id } });
};
/** 提交按钮 */
const submitForm = () => {
...
const handleView = (row?: QualityInspectionVO) => {
  // getExData(row);
  const _id = row?.id || ids.value[0];
  router.push({ name: 'QualityInspectionDetail', params: { id: _id } });
};

// const getExData = async (row) => {
//   batchIdLoading.value = true;
//   const res = await listPostharvestBatch({ pageNum: 1, pageSize: 10, batchId: row.batchId });
//   batchIdList.value = res.rows;
//   batchIdOptions.value = res.rows.map((item) => ({
...
//   }));
//   batchIdLoading.value = false;
//   form.value.batchName = batchIdList.value.find((item) => item.batchId == row.batchId).batchName;
// };
