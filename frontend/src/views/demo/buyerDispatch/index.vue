        ...
        <el-card shadow="hover" class="search-card">
          <el-form ref="queryFormRef" :model="queryParams" label-position="top" class="search-section">
            <!-- <el-form-item label="Dispatch ID" prop="dispatchId">
              <el-input v-model="queryParams.dispatchId" placeholder="Please enter Dispatch ID" clearable @keyup.enter="handleQuery" />
            </el-form-item> -->
            <el-form-item label="Dispatch ID" prop="dispatchId">
              <el-select
...
/** Add Button Action */
const handleAdd = () => {
  router.push({ name: 'BuyerDispatchAdd' });
};

/** Edit Button Action */
const handleUpdate = (row?: BuyerDispatchVO) => {
  const current: any = row || {};
  const _id = current.id || ids.value[0];
  if (isViewFlag(current)) {
    router.push({ name: 'BuyerDispatchDetail', params: { id: _id } });
  } else {
    router.push({ name: 'BuyerDispatchEdit', params: { id: _id } });
  }
};
