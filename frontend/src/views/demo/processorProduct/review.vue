...
const handleAdd = () => {
  reset();
  dialog.visible = true;
  dialog.title = "Review " + processorProductConfig.title;
}
...
const handleUpdate = (row?: ProcessorProductVO) => {
  ...
  if (row && canEdit(row)) {
    router.push({ name: 'ProcessorProductReview', params: { id: _productId } });
  } else {
...
const showApprovedComment = (row: ProcessorProductForm): boolean => {
  ...
  // 例如：只有已审批或已拒绝状态才展示
  // return row.status === String(ProcessorProductStatus.Approved) || row.status === String(ProcessorProductStatus.Reviewed);
  // return row.status != undefined && row.status != String(ProcessorProductStatus.Draft);
