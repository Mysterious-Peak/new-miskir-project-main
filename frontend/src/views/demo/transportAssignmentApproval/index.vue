        ...
        <el-card shadow="hover" class="search-card">
          <el-form ref="queryFormRef" :model="queryParams" label-position="top" class="search-section">
            ...
            <!-- <el-form-item label="Transporter ID" prop="transporterId">
              <el-input v-model="queryParams.transporterId" placeholder="Please enter Transporter ID" clearable @keyup.enter="handleQuery" />
            </el-form-item> -->
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          ...
          <!-- <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['demo:transportAssignment:edit']">
              Edit
            </el-button>
          </el-col>
          <el-col :span="1.5">
        ...
        <el-table-column label="Actions" align="center" fixed="right" class-name="action-column" width="110">
          <template #default="scope">
            <el-button
              ...
              icon="View"
              @click="router.push({ name: 'TransportAssignmentApprovalDetail', params: { id: scope.row.id } })"
              >View</el-button
            >
            <el-button
              v-if="!isViewFlag(scope.row)"
              ...
              icon="EditPen"
              @click="router.push({ name: 'TransportAssignmentApprovalApprove', params: { id: scope.row.id } })"
              >Edit</el-button
            >
          </template>
        </el-table-column>
...
const handleAdd = () => {
  reset();
  dialog.visible = true;
  dialog.title = 'Add Transport Assignment';
};

/** Edit / View Button Action */
const handleUpdate = async (row?: TransportAssignmentVO) => {
  reset();
  const current: any = row || {};
  const _id = current.id || ids.value[0];
  const res = await getTransportAssignment(_id);
  Object.assign(form.value, res.data);

  // 使用表格行里的名称，保证下拉框显示 name 而不是纯 ID
  if (current && form.value.batchId) {
    batchIdOptions.value = [
      {
  ...
  dialog.visible = true;
  dialog.title = isViewFlag(current) ? 'View Transport Assignment' : 'Edit Transport Assignment';
};
