        ...
        <el-table-column label="Actions" align="center" fixed="right" width="110" class-name="action-column">
          <template #default="scope">
            <el-button
              ...
              icon="EditPen"
              @click="router.push({ name: 'RenewProcessingLicense', params: { id: scope.row.licenseId } })"
              v-hasPermi="['demo:processingLicense:edit']"
              >Edit</el-button
            >
            <!-- <el-tooltip content="View" placement="top" v-if="scope.row.status !== String(processingLicenseStatus.Submitted)">
              <el-button link type="primary" icon="View" @click="handleView(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip content="Edit" placement="top" v-else>
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['demo:processingLicense:edit']"></el-button>
            </el-tooltip> -->
          </template>
        </el-table-column>
      </el-table>
...
const handleUpdate = async (row?: ProcessingLicenseVO) => {
  ...
  Object.assign(form.value, res.data);
  dialog.visible = true;
  dialog.title = 'Processing License';
};
