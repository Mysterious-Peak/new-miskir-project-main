        ...
        <el-card shadow="hover" class="search-card">
          <el-form ref="queryFormRef" :model="queryParams" label-position="top" class="search-section">
            <el-form-item label="Farmland ID" prop="farmlandId">
              <!-- <el-input v-model="queryParams.farmlandId" placeholder="Please enter Farmland ID" clearable @keyup.enter="handleQuery" /> -->
              <el-select
            ...
            <!-- <el-form-item label="Soil pH" prop="ph">
              <el-input v-model="queryParams.ph" placeholder="Please enter Soil pH" clearable @keyup.enter="handleQuery" />
            </el-form-item> -->
            <!-- <el-form-item label="Organic Matter (%)" prop="organicMatterPct">
              <el-input v-model="queryParams.organicMatterPct" placeholder="Please enter Organic Matter (%)" clearable @keyup.enter="handleQuery" />
            </el-form-item> -->
            <!-- <el-form-item label="Nitrogen (N) Level" prop="nLevel">
              <el-input v-model="queryParams.nLevel" placeholder="Please enter Nitrogen (N) Level" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="Phosphorus (P) Level" prop="pLevel">
              <el-input v-model="queryParams.pLevel" placeholder="Please enter Phosphorus (P) Level" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="Potassium (K) Level" prop="kLevel">
              <el-input v-model="queryParams.kLevel" placeholder="Please enter Potassium (K) Level" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="Notes" prop="notes">
              <el-input v-model="queryParams.notes" placeholder="Please enter Notes" clearable @keyup.enter="handleQuery" />
            </el-form-item>
...
<script setup name="SoilProfile" lang="ts">
import { delSoilProfile, listSoilProfile } from '@/api/demo/soilProfile';
...
import { soilProfileExtendedColumns, normalizeSoilProfileRecord } from '@/views/demo/soilProfile/constants';
...
/** Query Soil Profile List */
const getList = async () => {
  loading.value = true;
  const res = await listSoilProfile(queryParams.value);
...
/** Edit Button Action */
const handleUpdate = (row?: SoilProfileVO) => {
  const _soilProfileId = row?.soilProfileId || ids.value[0];
  router.push({
    path: `${route.path}/form`,
    query: {
      soilProfileId: String(_soilProfileId),
      title: 'Edit Soil Profile',
      description: 'Update soil profile record'
    }
  });
};

/** Delete Button Action */
const handleDelete = async (row?: SoilProfileVO) => {
  const _soilProfileIds = row?.soilProfileId || ids.value;
  await proxy?.$modal.confirm('Are you sure delete this data?').finally(() => (loading.value = false));
  await delSoilProfile(_soilProfileIds);
...
const handleExport = () => {
  proxy?.download(
    'demo/soilProfile/export',
    {
      ...queryParams.value
    },
    `soilProfile_${new Date().getTime()}.xlsx`
  );
};
...
const handleView = (row?: SoilProfileVO) => {
  ...
  router.push({
    ...
    query: {
      soilProfileId: String(_soilProfileId),
      mode: 'view',
      title: 'View Soil Profile',
      description: 'View soil profile record'
    }
  });
};
