    ...
    <el-card shadow="never">
      ...
      <el-form ref="processingLicenseFormRef" :model="form" :rules="rules" :disabled="isDetail" label-width="180px">
        ...
        <!-- <el-form-item label="Approval Comment" prop="approvedComment">
          <el-input v-model="form.approvedComment" type="textarea" placeholder="Please input Approval Comment" />
        </el-form-item> -->
      </el-form>
      <div class="form-footer">
        <el-button v-if="isDetail" @click="cancel">Close</el-button>
        <template v-else>
          <el-button icon="SendPlane" :loading="buttonLoading" type="primary" @click="submitForm">Submitted</el-button>
...
<script setup name="ProcessingLicenseReviewForm" lang="ts">
...
/** Load record for review/detail mode */
...
const submitForm = () => {
  processingLicenseFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      form.value.status = processingLicenseStatus.Submitted.toString();
