        ...
        <el-table-column label="Actions" align="center" fixed="right" width="110" class-name="action-column">
          <template #default="scope">
            <el-button
              ...
              icon="EditPen"
              @click="router.push({ name: 'ReviewProcessingLicense', params: { id: scope.row.licenseId } })"
            ...
            <el-button
              ...
              icon="View"
              @click="router.push({ name: 'ProcessingLicenseReviewDetail', params: { id: scope.row.licenseId } })"
              >View</el-button
            >
          </template>
        </el-table-column>
...
const handleUpdate = async (row?: ProcessingLicenseVO) => {
  ...
  Object.assign(form.value, res.data);
  dialog.visible = true;
  dialog.title = 'Review Processing License';
};
