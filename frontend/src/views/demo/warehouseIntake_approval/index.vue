...
<script setup name="WarehouseIntake Approval" lang="ts">
...
const handleAdd = () => {
  reset();
  dialog.visible = true;
  dialog.title = 'Add Warehouse Intake';
};
...
const handleUpdate = (row?: WarehouseIntakeVO) => {
  ...
  router.push({ name: 'WarehouseIntakeApprovalApprove', params: { id: _id } });
};
...
const handleView = (row?: WarehouseIntakeVO) => {
  const _id = row?.id || ids.value[0];
  router.push({ name: 'WarehouseIntakeApprovalDetail', params: { id: _id } });
};
