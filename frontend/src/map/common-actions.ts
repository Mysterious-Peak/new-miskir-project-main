...
import { profile_settings, settings_general } from './common/settings-options';
import type { Profile } from './valhalla';
...
export const updatePermalink = () => (_, getState) => {
  const { waypoints } = getState().directions;
  const { geocodeResults, maxRange, interval, generalize, denoise } = getState().isochrones;
...
export const downloadFile = ({ data, fileName, fileType }: { data: string | ArrayBuffer; fileName: string; fileType: string }) => {
...
export const filterProfileSettings = (profile: Profile, settings: PossibleSettings) => {
  const filteredSettings: {
    costing: Record<string, string | number | boolean | string[] | GeoJSON.GeoJSON[] | undefined>;
    directions: {
    ...
    if (profile in settings_general) {
      for (const item of settings_general[profile].numeric) {
        if (setting === item.param) {
          filteredSettings.costing[setting] = settings[setting as keyof PossibleSettings];
        }
      }
