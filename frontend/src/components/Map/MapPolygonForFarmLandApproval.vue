...
import { TERRA_POLYGON_MODE_CURSORS_GRABBING, TERRA_SELECT_MODE_CURSORS_GRABBING } from '@/components/Map/terraDrawCursors';
...
const initMap = () => {
  ...
  mapControl = new MaplibreTerradrawControl({
    ...
    modeOptions: {
      polygon: new TerraDrawPolygonMode({
        cursors: TERRA_POLYGON_MODE_CURSORS_GRABBING,
        styles: { ...FARM_LAND_TERRA_POLYGON_STYLES }
      }),
      select: new TerraDrawSelectMode({
        cursors: TERRA_SELECT_MODE_CURSORS_GRABBING,
  ...
  map.once('load', () => {
    ...
    if (draw) {
      ...
      map.resize();
      detachMapCanvasCursor = attachUniformMapCanvasCursor(map.getCanvas(), { idle: 'grabbing' });
      mapReady = true;
