  ...
  <div class="component-upload-image">
    ...
    <div v-if="showTip" class="el-upload__tip">
      ...
      <template v-if="fileSize">
        not exceeding <b style="color: #f56c6c">{{ fileSize }}MB</b> in size
      </template>
      <template v-if="fileType">
        in the format of <b style="color: #f56c6c">{{ fileType.join('/') }}</b>
      </template>
    </div>
...
const handleBeforeUpload = (file: any) => {
  ...
  if (!isImg) {
    proxy?.$modal.msgError(`File format Error, Please upload in the format of ${props.fileType.join('/')}!`);
    return false;
  }
  if (file.name.includes(',')) {
    proxy?.$modal.msgError('The file name is incorrect and cannot contain English commas!');
    return false;
  }
  if (props.fileSize) {
    const isLt = file.size / 1024 / 1024 < props.fileSize;
    if (!isLt) {
      proxy?.$modal.msgError(`File not exceeding ${props.fileSize} MB in size!`);
      return false;
    }
  }
...
// 禁用状态下隐藏上传按钮
:deep(.el-upload.is-disabled .el-upload--picture-card) {
  display: none;
}
</style>
